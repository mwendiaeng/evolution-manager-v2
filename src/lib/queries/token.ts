...
export const saveToken = async (params: SaveCredentialsParams) => {
  ...
  if (params.facebookAppId)
    localStorage.setItem(TOKEN_ID.FACEBOOK_APP_ID, params.facebookAppId);
  if (params.facebookConfigId)
    localStorage.setItem(TOKEN_ID.FACEBOOK_CONFIG_ID, params.facebookConfigId);
  if (params.facebookUserToken)
    localStorage.setItem(
      TOKEN_ID.FACEBOOK_USER_TOKEN,
      params.facebookUserToken,
    );
  if (params.clientName)
    localStorage.setItem(TOKEN_ID.CLIENT_NAME, params.clientName);
};
