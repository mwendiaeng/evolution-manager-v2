...
export function MessageBubble({
  ...
  const _baseStyles = {
    className:
      "group relative flex max-w-[60%] flex-col break-words rounded-[16px] after:absolute after:bottom-[12px] after:border-solid after:border-[8px]",
  };

  return (
    <div
      className={cn(
        _baseStyles.className,
        fromMe ? "message-bubble-from-me" : "message-bubble-from-other",
        className,
      )}
...
MessageBubble.Content = function MessageContent({
  children,
  className,
  fromMe,
}: {
...
MessageBubble.Footer = function MessageFooter({
  children,
  className,
  fromMe,
}: {
  ...
  return (
    <div
      className={cn(
        "flex justify-end gap-1 text-xs text-muted-foreground",
        className,
      )}
      style={{
        color: fromMe ? fromMeForegroundColor : fromOtherForegroundColor,
...
MessageBubble.Actions = function MessageActions({
  children,
  className,
}: {
  children: React.ReactNode;
  ...
  return (
    <div
      className={cn(
        "flex flex-col border-t border-black/10 dark:border-white/10",
        className,
      )}
    >
      {children}
