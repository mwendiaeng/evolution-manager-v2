...
import { getContactBgColor, getContactColor } from "@/utils/contact-colors";
    ...
    >
      <div
        className={`absolute h-full w-1 rounded-l-lg ${
          quotedMessage.key.fromMe
            ...
            : getContactBgColor(
                quotedMessage.key.participant ?? chat?.remoteJid ?? "",
              )
        }`}
      />
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-2 p-2 pl-3">
        ...
        >
          {quotedMessage.key.fromMe
            ? "Você"
            : (quotedMessage.key.participantPushName ?? chat?.pushName)}
        </span>
        {renderQuotedMessage()}
      </div>
    </div>
